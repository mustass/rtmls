use burn::{nn::loss::CrossEntropyLoss, optim::{GradientsParams, Sgd, Optimizer, SgdConfig, adaptor::OptimizerAdaptor}, tensor::Data};
...
use burn::tensor::{
    backend::{AutodiffBackend, Backend}, Int, Tensor,
};
...
impl<B: AutodiffBackend>  HotNotDogClassifier<B>  {
    pub fn new() -> Self {
        ...
        Self { model, normalizer, optimizer:optim }
    }
    ...
    B::IntElem: Into<usize>,{
        ...
        let arg_max = output.argmax(1).into_scalar().into() as usize;
...
pub fn  load_image<B: Backend>(path: &str) -> Tensor<B, 4> where Data<<B as Backend>::FloatElem, 3>: From<[[[f32; 224]; 224]; 3]> {
